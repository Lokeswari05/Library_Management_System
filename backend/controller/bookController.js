...
export async function issueManualBooks(req, res){
    try{
        ...
        const student = await User.findOne({ role: studentDetails.rollNumber });
        if(!student){
        ...
        if(validBooks.length === 0){
            return res.status(400).json({
                message:"Please add at least one valid manual book entry with the book code and a due date."
            })
        }
...
// 2. get all the manual issues (admin)
export async function getIssues(req, res){
    try{
        const issues = (await Issue.find({ })).toSorted({ createdAt: -1});
        res.status(200).json({
            success: true,
            issues
        });
...
// 3. get all the manual issues for logged-in student
export async function getStudentIssues(req, res){
    try{
        ...
        console.error("Error fetching student's manual issues:", error);
        res.status(500).json({
            success: false,
            message: "Error fetching student's manual issues.", error: error.message
        });
    }
}
...
export async function clearFine(req, res){
    try{
        ...
        res.status(200).json({
            success: true,
            message: "Manual fine cleared successfully.",
            issue
        });
    }
    catch(error){
